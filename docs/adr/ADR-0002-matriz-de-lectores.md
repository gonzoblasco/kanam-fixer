...
| Local (real) | VoiceOver | la que provea el asset de Guidepup para la version del SO | macOS | **Limitado (declarado, 2026-09-15).** En Darwin 27 el real no es verificable: el workaround probado con Full Disk Access no estabilizo (ver nota). Reportado en guidepup/guidepup#149. |
| Local (real) | NVDA | - | Windows | Fuera de alcance de 0.0.1 (declarado en el BRIEF). |

**Nota sobre el plano real (2026-09-15, resultado de S3):** la version del lector real **no la elige el proyecto**: la determina el asset que Guidepup publique para la version mayor de Darwin del host. Es una dependencia dura y quedo demostrada, no teorica. El plano virtual, en cambio, corre en cualquier SO. Esta asimetria es la razon por la que S4 concluye que el plano real es el nucleo del producto y el virtual una red barata, no un sustituto.

**Y el intento acotado lo confirmo en carne propia (2026-09-15, ultima iteracion):** con el Full Disk Access de Gonzo + el workaround local, el plano real **arranco y se detuvo limpio** en Darwin 27, pero se revelaron dos cosas que cambian el cuadro: (1) la "frase real" capturada en la iteracion anterior era en realidad el **dialogo del sistema de AirPlay** que macOS muestra al arrancar VoiceOver, no el contenido del fixture - VoiceOver anuncia contenido solo cuando hay una **aplicacion real enfocada** (un browser), y los tests corren sobre un DOM simulado (jsdom), asi que el paso a `next()` devuelve frases vacias; (2) el `activate` de Guidepup es intermitente en este SO. El camino al plano real verificable en tests pasa por **@guidepup/playwright** (browser real, ya declarado en S1) + criterio de fin de recorrido por plano. Hasta eso, la fila queda declarada como limitacion.
